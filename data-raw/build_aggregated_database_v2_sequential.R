# Build Aggregated Patent Database v2 - ENHANCED PROGRESS TRACKING
#
...
load_istrax_data <- function(toflow, country_codes = NULL,
                             countrymap_data = countrymap,
                             istrax_dir_path = istrax_dir) {
  istrax_path <- file.path(istrax_dir_path, paste0(toflow, ".fst"))
...
run_with_firms <- function(data, toflow, aggregate_fn) {
  ...
  # cat("      - Processing 'No firm' (all patents)...\n")
  ...
  for (fname in firm_names) {
    ...
    if (nrow(data_firm) == 0) {
      # cat("        ⚠ No patents for this firm, skipping\n")
      next
    }
...
#   # cat(sprintf("      Combining %d result sets...\n", length(batch_list)))
#   combined <- dplyr::bind_rows(batch_list)
#   # cat(sprintf("      Combined: %d total rows\n", nrow(combined)))

#   if (file.exists(output_path)) {
#     # cat("      Appending to existing file...\n")
#     existing <- arrow::read_parquet(output_path)
#     combined <- dplyr::bind_rows(existing, combined)
#     rm(existing)
#     gc(verbose = FALSE)
#     # cat(sprintf("      New total: %d rows\n", nrow(combined)))
#   }
...
process_toflow_part1 <- function(toflow, p) {
  batch <- list()
  for (j in seq_along(all_country_selections)) {
    ...
    if (!is.null(result) && nrow(result) > 0) {
      ...
      batch[[length(batch) + 1L]] <- result
    }
  }
  p(sprintf("Part 1: %s", toflow))
  if (length(batch) > 0) dplyr::bind_rows(batch) else NULL
}

part1_results <- with_progress({
  p <- progressor(along = toflows)
  future_lapply(toflows, process_toflow_part1, p = p, future.seed = TRUE)
})

part1_combined <- dplyr::bind_rows(Filter(Negate(is.null), part1_results))
total_rows_tech <- nrow(part1_combined)
arrow::write_parquet(part1_combined, output_by_tech)
rm(part1_results, part1_combined); gc(verbose = FALSE)
...
process_toflow_part2 <- function(toflow, p) {
  batch <- list()

  data_full <- load_istrax_data(toflow, country_codes = NULL)
  if (is.null(data_full) || nrow(data_full) == 0) return(NULL)

  for (j in seq_along(tech_selections)) {
  ...
  p(sprintf("Part 2: %s", toflow))
  if (length(batch) > 0) dplyr::bind_rows(batch) else NULL
}

part2_results <- with_progress({
  p <- progressor(along = toflows)
  future_lapply(toflows, process_toflow_part2, p = p, future.seed = TRUE)
})

part2_combined <- dplyr::bind_rows(Filter(Negate(is.null), part2_results))
total_rows_country <- nrow(part2_combined)
arrow::write_parquet(part2_combined, output_by_country)
rm(part2_results, part2_combined); gc(verbose = FALSE)
